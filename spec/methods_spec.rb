...
RSpec.describe Enumerable do
  ...
  describe '#my_select' do
    ...
    it 'returns are element vogals.' do
      a = %w[a b c d e f]
      expect(a.my_select { |v| v =~ /[aeiou]/ }).to eql(%w[a e])
    end
  ...
  describe '#my_any?' do
    ...
    it 'Returns true if any element is nil' do
      expect(['antsdfassfas', 'bear', nil].my_any?(&:nil?)).to eql(false)
    end

    it 'Returns true if any element is nil' do
      expect(%w[nil bear 12].my_any?(&:nil?)).to eql(false)
    end
  ...
  describe '#my_count' do
    ...
    it 'Returns the count or the elements that the block returns true' do
      expect([1, 2, 3, 4, 5, 6].my_count(&:even?)).to eql(3)
    end
  end
  ...
  describe '#my_each_with_index' do
    ...
    it 'returns the Index of the array numbers' do
      expect(array.my_each_with_index { |_x, i| array[i] = i }).to eql([0, 1, 2])
    end
  ...
  describe '#my_inject' do
    ...
    it 'applies a multiplication especified by a block' do
      expect(array.my_inject { |sum, n| sum * n }).to eql(151_200)
    end
